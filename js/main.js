...
form.addEventListener("submit", (e) => {
  ...
  const errorCampos = () => {
    if (botonIda.value === "" || botonIdaVuelta.value === "" || seleccionOrigen === "" || seleccionDestino === "" || inputPasajeros === "" || inputPartida === "" || inputRegreso === "") {
      mensajeError.innerHTML = "Por favor, complete todos los campos";
      mensajeError.className = "text-danger mt-2";
    } else (inputPartida < fechaHoy || fechaRegreso < inputPartida); {
      mensajeError.innerHTML = "Por favor, ingrese una fecha válida";
      mensajeError.className = "text-danger mt-2";
    }
  };

  const seguirSeleccion = errorCampos();
  if (seguirSeleccion == false) {
